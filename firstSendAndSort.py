...
# проходки, тк есть много групп, не подходящих по тематике, но тк среди них много закрытых групп, поэтому уйдет слишком
...
#   - пост опубликуется (проверка по id поста среди предложенных постов группы) - записываем инфу
...
import requests
from data import my_token, V, dima_token
import os
...
try:
    ...
    for attach in src["attachments"]:
        ...
        if attach["type"] == "link":
            ...
            attachment += attach["type"] + str(attach[attach["type"]]["owner_id"]) + '_' + str(
...
# для проверки
# url = f"https://api.vk.com/method/wall.post?owner_id=-215267285&message={message}&attachment={attachment}&access_token={my_token}&v={V}"
